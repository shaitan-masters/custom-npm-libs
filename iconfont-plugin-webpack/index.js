...
Plugin.prototype.getOptions = function(options) {
  ...
  const cssTemplate = 'function' === typeof opts.cssTemplate ? opts.cssTemplate : require('./src/template');
  return {
    src: src,
    dest: {
      font: dest.font,
  ...
  return new Promise(function(resolve, reject) {
    ...
    .then(function(args) {
      ...
      const files = ['svg', 'ttf', 'woff', 'eot'].map(function(type) {
        ...
        const filePath = context.options.dest.font.replace(/\[family\]/g, family).replace(/\[type\]/g, type);
        return { path: filePath, contents: buffer };
      }, []);
      return { files: files, unicodes: args.unicodes, ttf };
    })
    .then(async args => {
      await woff2.init();
      const ttf = args.ttf;
      ...
      const filePath = context.options.dest.font.replace(/\[family\]/g, family).replace(/\[type\]/g, 'woff2');
      const font = {
        path: filePath,
        contents: buffer
      };
      args.files.push(font);
